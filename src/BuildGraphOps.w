...
pub fn build_graph_embed_object_files(root: &str, target: &BuildGraphTarget) -> i32:
    ...
    var has_linux_aarch64_runtime = false
    ...
    for ii in 0..target.inputs.len() as i32:
        ...
        if sym == "rt_linux_aarch64_o":
            has_linux_aarch64_runtime = true
    ...
    // object as an input and relies on this zero-fill for the rest. This list
    // is the same list as build.w's embedded_platform_symbols() and Link.w's
    // externs; a platform missing here surfaces far away as
    // `undefined symbol: with_embedded_rt_<platform>_o_start` at the cross
    // link (the windows-aarch64 seed, missing linux_aarch64).
    ...
    if not has_linux_aarch64_runtime:
        asm_text = asm_text ++ build_graph_emit_empty_embedded_blob("rt_linux_aarch64_o")
